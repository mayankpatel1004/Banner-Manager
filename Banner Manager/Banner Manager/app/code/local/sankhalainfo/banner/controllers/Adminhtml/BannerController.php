...
class Sankhalainfo_Banner_Adminhtml_BannerController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Item Manager'), Mage::helper('adminhtml')->__('Banner Manager'));
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				try {	
					...
					$arrInfo = $uploader->save($path, $_FILES['filename']['name'] );
					
					$strFile = $arrInfo['file'];
					
					$strFileName = $arrInfo['path'].$arrInfo['file'];
					 
					$objImage = new Varien_Image($strFileName);
					$objImage->resize(936,425);
					$objImage->save($path, 'thumb_'.$strFile);
